class UserSerializer < ActiveModel::Serializer
  ...
  def tandems
    r = []
    unless object.tandems.nil?
      object.tandems.each do |c|
        t = if (current_user && object == current_user) || (current_user && current_user.admin?)
              {
                id: c.id,
                inviter_grudges_invitee_for: c.inviter_grudges_invitee_for,
                invitee_grudges_inviter_for: c.invitee_grudges_inviter_for,
                disabled_by: c.disabled_by,
                invitation_type: c.invitation_type,
                inviter_id: c.inviter_id,
                invitee_id: c.invitee_id,
                invitee_name: c.invitee_name,
                invitee_email: c.invitee_email,
                invitation_accepted_at: c.invitation_accepted_at,
                invitee_participates: c.invitee_participates,
                inviter_code: c.inviter_code,
                invitee_code: c.invitee_code
              }
            else
              {
                id: c.id,
